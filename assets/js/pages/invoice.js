// ============================================================
// invoice.js - إنشاء صفحات فاتورة متعددة بحجم A5 ثابت
// المسار الجديد: assets/js/pages/invoice.js
// ============================================================
...
(function () {
    ...
    document.body.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:#f1f5f9;font-family:sans-serif;text-align:center;padding:20px;">
        <div>
          ...
          <button onclick="window.location.href='bookings.html'" style="margin-top:20px;background:#0f172a;color:#fff;border:none;padding:10px 30px;border-radius:12px;font-weight:bold;cursor:pointer;">
  ...
  function getInvoiceNumber(id) {
    return String(id || Math.floor(Math.random() * 1000000)).padStart(6, "0");
  }
  ...
  function createInvoicePage(pageItems, pageIndex, totalPages, grandTotal) {
      ...
      if (item) {
        ...
        itemsHtml += `
          <tr style="height: 8mm;">
            ...
            <td style="text-align:right; padding-right:6px; font-size:11px; border: 1px solid #420a09; ${depositStyle}">${escapeHtml(item.name || "صنف")}</td>
    ...
    return `
      <div class="invoice-page">
        <div class="watermark">
          <img src="assets/images/watermark.png" alt="Watermark" />
        </div>
        ...
        <div class="invoice-inner">
          ...
          <div class="header" style="align-items: center;">
            <div class="header-right">
              <div class="title">${escapeHtml(r.name || "مطابخ شواطئ عدن")}</div>
              <div class="subtitle">${escapeHtml(r.subtitle || "للحجوزات الولائم والمناسبات")}</div>
              <div class="phone">
                <i class="fas fa-phone"></i>
                <span>${escapeHtml(r.phone || "0550724459")}</span>
              </div>
            </div>

            <div class="logo-center">
              ${r.logo
                ? `<img src="${escapeHtml(r.logo)}" alt="شواطئ عدن" class="main-logo" />`
                : `<img src="assets/images/logo.png" alt="شواطئ عدن" class="main-logo" />`}
            </div>
            ...
            <div class="social-box">
              ...
              <div class="social-text">${escapeHtml(r.social || "مطابخ شواطئ عدن")}</div>
              <div class="social-text" style="font-size:14px;font-weight:bold;color:black;">${r.social ? escapeHtml(r.social.toUpperCase()) : "SHAWATI ADEN KITCHENS"}</div>
            </div>
          </div>
          ...
          <div class="customer-box" style="margin-top: 1mm;">
            <div class="customer-row">
              <div class="customer-field">
                ...
                <span class="customer-value" style="min-width: 45mm;">${escapeHtml(data.customer?.name || "")}</span>
              </div>
              <div class="customer-field">
                <span class="customer-label">الجوال :</span>
                <span class="customer-value" style="min-width: 30mm;">${escapeHtml(data.customer?.phone || "")}</span>
              </div>
            </div>
            <div class="customer-row">
              <div class="customer-field">
                <span class="customer-label">اليوم :</span>
                <span class="customer-value" style="min-width: 20mm;">${escapeHtml(data.booking?.day || "")}</span>
              </div>
              <div class="customer-field">
                <span class="customer-label">الساعة :</span>
                <span class="customer-value" style="min-width: 20mm;">${escapeHtml((data.booking?.time || "").replace("صباحاً", "").replace("مساءً", "").trim())} ${escapeHtml(data.booking?.deliveryTime || "")}</span>
              </div>
              <div class="customer-field">
                <span class="customer-label">التاريخ :</span>
                <span class="customer-value" style="min-width: 25mm;">${escapeHtml(data.booking?.date || "2026 / /")}</span>
              </div>
            </div>
            <div class="customer-row">
              <div class="customer-field" style="flex:1.5;">
                <span class="customer-label">موقع التوصيل :</span>
                <span class="customer-value" style="min-width: 30mm;">${escapeHtml(data.customer?.location || "")}</span>
              </div>
              <div class="customer-field" style="flex:1;">
                <span class="customer-label">العلامة :</span>
                <span class="customer-value" style="min-width: 20mm;">${escapeHtml(data.booking?.mark || "")}</span>
              </div>
            </div>
          </div>
          ...
          <div style="margin: 0 5mm 2mm 5mm; border: 1.5px solid #420a09; border-radius: 6px; height: 28mm; overflow: hidden; padding: 2mm 4mm; background-color: #fff; position: relative;">
            ...
            <div style="margin-top: 5mm; font-size: 11px; font-weight: 700; color: #420a09; line-height: 1.6; text-align: right; white-space: pre-wrap; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis;">${escapeHtml(data.notes || "")}</div>
          </div>
          ...
          <div style="margin: 0 5mm; border: 1.5px solid #420a09; border-radius: 6px; padding: 2.5mm 4mm; background-color: #fff; display: flex; align-items: center; justify-content: space-between;">
            ...
            <span style="font-size: 9px; font-weight: 800; color: #420a09; width: 30%; text-align: left; opacity: 0.8; line-height: 1.4;">
              المدفوع: <span style="color:#27ae60;">${formatCurrency(data.deposit)}</span><br>المتبقي: <span style="color:#c0392b;">${formatCurrency(Math.max(0, grandTotal - (data.deposit || 0)))}</span>
            </span>
          </div>
          ...
          <div class="footer-content">
            <div class="qr-box">
              <img src="assets/images/location-qr.png" class="qr-image" alt="Location QR" />
            ...
            <div class="footer-center" style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%;">
              <div style="display:flex; justify-content:space-between; width:100%; align-items:center; padding:0 4mm; margin-bottom:4px;">
                <div class="address-location" style="display:flex; align-items:center; justify-content:center; gap:3mm; text-align:right; margin:0;">
                  ...
                  <div class="address" style="text-align:right; margin:0;">
                    <p>${escapeHtml(r.address || "جدة . شارع جاك - جوار كودو")}</p>
                  </div>
                </div>
                <div style="display:flex; flex-direction:column; gap:4px;">
                  <div class="contact-row" style="margin:0; justify-content:flex-start;">
                    <i class="fab fa-whatsapp"></i>
                    <span>${escapeHtml(r.delivery || r.phone || "0550724459")}</span>
                  </div>
                  <div class="contact-row" style="margin:0; justify-content:flex-start;">
                    <i class="fas fa-phone"></i>
                    <span>${escapeHtml(r.phone || "0566869958")}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="payment-box" style="margin-top:4px; display:flex; align-items:center; justify-content:center; gap:4mm;">
                ...
                <span class="payment-line" style="display:inline-block; width:42mm; height:7mm; background-color:#fafbf6; border:1px solid #420a09; border-radius:2mm; text-align:center; font-weight:bold; font-size:11px; color:#420a09; line-height:7mm;">
                  ${escapeHtml(data.paymentMethod || "نقدي")}
                </span>
              </div>
            </div>

            <div class="qr-box">
              <img src="assets/images/whatsapp-qr.png" class="qr-image" alt="Whatsapp QR" />
  ...
  function renderInvoice() {
    ...
    // بناء القائمة الكاملة للأصناف مع إضافة تأمين الصحون في البداية
    let fullItems = [];
    ...
    fullItems = fullItems.concat(
      items.map((item) => ({
        name: item.name || "صنف",
        qty: item.qty || item.quantity || 1,
        price: item.price || item.unit_price || 0,
        total: item.total || (item.price * (item.qty || 1)) || 0,
        isDeposit: false
      }))
    );

    // حساب الإجمالي الكلي (يشمل التأمين)
    const grandTotal = fullItems.reduce(
    ...
    if (!wrapper) {
      console.error("❌ pagesWrapper غير موجود في HTML");
      return;
    }

    wrapper.innerHTML = allPagesHtml;
    console.log("✅ تم إنشاء", pages.length, "صفحة فاتورة");
  }
  ...
  window.shareInvoiceWhatsApp = async function () {
    ...
    try {
      ...
      const customerName = data.customer?.name
        ? data.customer.name.replace(/\s+/g, "")
        : "عميل";
      ...
      if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
        ...
        alert("تم تحميل الفاتورة بصيغة PDF بحجم A5. يمكنك إرسالها يدويًا.");
      }
    } catch (error) {
  ...
  // دالة تشخيص
  window.__invoiceDebug = function () {
    const wrapper = document.getElementById("pagesWrapper");
    console.log("pagesWrapper موجود:", !!wrapper);
    console.log("عدد الصفحات:", wrapper ? wrapper.children.length : 0);
